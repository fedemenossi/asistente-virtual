...
def test_manual_invoice_preview_is_transient_and_uses_patient_fiscal_profile(client, db_session):
    ...
    preview = client.post("/t/billing/manual/preview", data={"csrf_token": _csrf(form.text), "patient_id": patient_id, "receiver_name": "Juan Perez", "receiver_document_type": "DNI", "receiver_document_number": "12345678", "receiver_iva_condition": "consumidor_final", "receiver_email": "juan@example.com", "item_id": item_id, "amount": "1250.50", "service_start": "2026-07-17", "service_end": "2026-07-18", "sale_condition": "Transferencia", "send_email": "on"})
...
def test_manual_invoice_preview_uses_selected_catalog_value_and_keeps_diagnosis(client, db_session):
    ...
    form = client.get("/t/billing/manual/new")
    assert "Valor personalizado" in form.text
    ...
    preview = client.post(
        ...
        data={
            ...
            "amount_source_item_id": procedure_id,
            "amount": "9999.99",
    ...
    assert preview.status_code == 200
    assert "2500.00" in preview.text
